...
def calculate_rolling_log_diff(data_config, commodity, window=28):
    """
    ...
    """
    param_historical_fuels, _ = load_data(data_config)

    if commodity not in param_historical_fuels.columns:
        raise ValueError(f"Commodity {commodity} not found in historical data")
